...
def _maybe_start_summaries_daemon():
    ...
    # Point d'entrée installé avec le paquet, dans le même environnement que
    # l'interpréteur d'uvicorn (voir [project.scripts] dans pyproject.toml)
    daemon = shutil.which("oceens-summaries", path=sysconfig.get_path("scripts"))
    if daemon is None:
        logger.error("Point d'entrée oceens-summaries introuvable : daemon non lancé")
        return None
...
def run():
    """Point d'entrée `oceens` (voir [project.scripts] dans pyproject.toml)."""
    uvicorn.run(
        "oceens.main:app",
        host="0.0.0.0",
        port=8000,
