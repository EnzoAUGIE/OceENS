...
The repository has no automated test suite and no CI (the first tests are #85,
the CI is #78). This procedure runs entirely **outside the process**: start
...
Commands are given for **Windows (PowerShell)** then for **macOS /
...
The startup logs create the tables, insert the demo data and contain no error
...
The three cases are, in order: invalid `AUTH_MODE`; `ENTRA_*` missing, without
`.env`; `SECRET_KEY` missing in `entra` mode, without `.env`.
...
(see [the `SECRET_KEY` rule](../README.md#the-secret_key-rule)).
...
Each student gets their own key at <https://locallm.mde.epf.fr> by signing in
...
the interpreter path and the way to set the variable change.
